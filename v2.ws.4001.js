...
app.use(async(ctx, next)=>{
  ...
  //ctx.status=404;
  //ctx.body="todd's ws server";
  //ctx.body=ctx.request.query;
  console.log(util.inspect(test1Ws.nsps['/'], true, 1, true));
  switch(ctx.request.query.type){
    case 'sockets':
      var ret=new Set();
      ctx.body=test1Ws.nsps['/'].sockets;
      for(let k in test1Ws.nsps['/'].sockets){
        ret.add(k);
      }
      ctx.body=Array.from(ret);
      break;
    case 'rooms':
      if(ctx.request.query.id && test1Ws.nsps['/'].sockets[ctx.request.query.id]){
        ctx.body=test1Ws.nsps['/'].sockets[ctx.request.query.id].rooms;
      }else{
        ctx.body='no such id:'+ctx.request.query.id;
      }
      break;
    case 'kill':
      if(ctx.request.query.kill && test1Ws.nsps['/'].sockets[ctx.request.query.kill]){
        test1Ws.nsps['/'].sockets[ctx.request.query.kill].disconnect();
      }
      var ret=new Set();
      ctx.body=test1Ws.nsps['/'].sockets;
      for(let k in test1Ws.nsps['/'].sockets){
